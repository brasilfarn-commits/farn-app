...
function handleLogout() {
    if (typeof wfSair === 'function') wfSair();
...
function fichaGeralInicializar() {
    const selProj = document.getElementById('ficha-geral-selecao-projeto');
    if (!selProj) return;
    selProj.innerHTML = '<option value="">Selecione o projeto...</option>';
    projetos.filter(p => (p.status || 'Em Andamento') === 'Em Andamento').forEach(p => {
        selProj.innerHTML += '<option value="' + p.nome + '">' + p.nome + (p.responsavel ? ' - ' + p.responsavel : '') + '</option>';
    });
    const selTurma = document.getElementById('ficha-geral-selecao-turma');
    if (selTurma) selTurma.innerHTML = '<option value="">Selecione a turma...</option>';
...
function fichaGeralOnProjetoChange() {
    const projetoNome = document.getElementById('ficha-geral-selecao-projeto').value;
    const selTurma = document.getElementById('ficha-geral-selecao-turma');
    selTurma.innerHTML = '<option value="">Selecione a turma...</option>';
    if (projetoNome) {
        turmas.filter(t => t.projeto === projetoNome).forEach(t => {
    ...
    fichaGeralOnTurmaChange();
}

function fichaGeralOnTurmaChange() {
    const projeto = document.getElementById('ficha-geral-selecao-projeto').value;
    const turma = document.getElementById('ficha-geral-selecao-turma').value;
    const conteudo = document.getElementById('ficha-geral-conteudo');
    if (projeto && turma) {
        if (conteudo) conteudo.style.display = '';
        fichaGeralRenderList();
    } else {
        if (conteudo) conteudo.style.display = 'none';
    }
}
...
function fgCampo(rotulo, valor) {
    ...
    return '<div style="border:1px solid #e2e8f0;border-radius:8px;padding:6px 9px;background:#f8fafc;min-width:0">'
        + '<div style="font-size:8.5px;font-weight:700;color:#64748b;text-transform:uppercase;letter-spacing:.4px">' + escHTML(rotulo) + '</div>'
...
function fgSecao(icone, titulo, conteudoHtml) {
    return '<div style="margin-bottom:14px">'
        ...
        + '<span style="font-size:12px;font-weight:800;color:#0f172a;text-transform:uppercase;letter-spacing:.6px">' + escHTML(titulo) + '</span></div>'
        + conteudoHtml + '</div>';
}
...
function fgNotaPill(rotulo, nota, cor) {
    return '<div style="flex:1;min-width:110px;border:1px solid ' + cor + '44;background:' + cor + '12;border-radius:10px;padding:8px;text-align:center">'
        ...
        + '<div style="font-size:9.5px;font-weight:700;color:#475569;text-transform:uppercase;letter-spacing:.4px;margin-top:2px">' + escHTML(rotulo) + '</div></div>';
}
...
function fgTabelaNotas(listaAulas, notas) {
    ...
    listaAulas.forEach(a => {
        ...
        const notaVal = (n === undefined || n === null || n === '') ? '—' : escHTML(String(n).replace('.', ','));
        rows += '<tr style="border-bottom:1px solid #eef2f7">'
            ...
            + '<td style="padding:5px 6px;font-size:11px;font-weight:600;color:#0f172a">' + escHTML(a.disciplina || '—') + '</td>'
            + '<td style="padding:5px 6px;font-size:10.5px;color:#64748b">' + escHTML((a.conteudo || '').substring(0, 42)) + '</td>'
...
function fgFOLista(lista, positivo) {
    ...
    lista.forEach(fo => {
        html += '<div style="display:flex;justify-content:space-between;gap:8px;background:' + cores[1] + ';border:1px solid ' + cores[2] + ';border-radius:8px;padding:5px 8px;margin-top:4px;font-size:11px">'
            ...
            + '<span style="color:#475569;flex:1;text-align:right">' + escHTML(fo.obs || '') + '</span></div>';
    });
    return html;
}
...
function fichaGeralCarregarAulas(projeto, turma) {
    ...
    return dbFirestore.collection('aulas').get().then(snap => {
        snap.forEach(doc => {
            const a = doc.data();
            a._id = doc.id;
...
async function fichaGeralDadosAluno(c) {
    ...
    let presencas = [];
    try {
        const doc = await dbFirestore.collection('avaliacoesAlunos').doc(String(c.cpf)).get();
        if (doc.exists) av = Object.assign(av, doc.data() || {});
    } catch (e) { console.error('Erro ao carregar avaliacao:', e); }
    try {
        const snap = await dbFirestore.collection('presencasAlunos').where('cpf', '==', c.cpf).get();
...
function fichaGeralFolha(c, calc, fotoSrc, idx, aulas) {
    ...
    const rodape = '<div style="display:flex;justify-content:space-between;align-items:center;border-top:1px solid #e2e8f0;padding-top:10px;margin-top:12px;font-size:10px;color:#94a3b8">'
        ...
        + '<button class="fg-somente-tela btn-outline btn-sm" onclick="fichaGeralImprimirUma(' + idx + ')"><i class="fa-solid fa-print"></i> Imprimir</button>'
        + '</div>';
    ...
    return '<div class="fg-quebra" style="background:#fff;border-radius:14px;box-shadow:0 12px 34px rgba(2,6,23,.14);border:1px solid #e2e8f0;padding:24px 26px;max-width:800px;margin:0 auto;color:#0f172a;font-family:Inter,Arial,sans-serif">'
        ...
        + '<div style="font-size:11px;font-weight:700;color:#0ea5e9;margin-top:4px"><i class="fa-solid fa-layer-group" style="margin-right:4px"></i>Projeto: ' + escHTML(fichaGeralProjeto || c.projeto) + '  •  Turma: ' + escHTML(fichaGeralTurma || c.turma) + '</div>'
        + '</div></div>'
        ...
        + '<div><div style="font-size:17px;font-weight:800;color:#0f172a">' + escHTML(nome) + '</div>'
        + '<div style="font-size:11px;color:#475569;margin-top:2px">Matricula: ' + escHTML(mat) + '  •  CPF: ' + escHTML(cpf) + '  •  Turma: ' + escHTML(fichaGeralTurma || c.turma) + '</div></div>'
        + '<span style="background:' + stCor[1] + ';color:' + stCor[0] + ';border:1px solid ' + stCor[2] + ';border-radius:20px;padding:4px 12px;font-size:11px;font-weight:800">' + escHTML(st) + '</span>'
        + '</div>'
        + secoes.join('')
        + assinaturas
        + rodape
...
async function fichaGeralRenderList() {
    ...
    const projeto = document.getElementById('ficha-geral-selecao-projeto').value;
    const turma = document.getElementById('ficha-geral-selecao-turma').value;
    const countEl = document.getElementById('ficha-geral-count');
    fichaGeralProjeto = projeto;
    fichaGeralTurma = turma;
    fichaGeralAulasCache = null;
    fichaGeralCache = [];
    ...
    if (!filtrados.length) {
        lista.innerHTML = '<div class="empty-state" style="text-align:center;padding:40px;color:#64748b"><i class="fa-solid fa-folder-open" style="font-size:40px;color:#cbd5e1;display:block;margin-bottom:12px"></i>Nenhum cadastro encontrado para o projeto e turma selecionados.</div>';
        return;
    }

    lista.innerHTML = '<div style="text-align:center;color:#64748b;padding:24px">Carregando fichas... <i class="fa-solid fa-spinner fa-spin"></i></div>';
    ...
    try {
        ...
        filtrados.forEach((c, i) => {
            getFoto(c.cpf).then(src => {
                const el = document.getElementById('fg-foto-' + i);
                if (el && src) {
                    el.innerHTML = '<img src="' + src + '" alt="Foto 3x4" style="width:86px;height:114px;object-fit:cover;border-radius:8px;border:2px solid #0ea5e9">';
                }
            }).catch(() => {});
        });
    } catch (e) {
        console.error('Erro ao renderizar ficha geral:', e);
        lista.innerHTML = '<div style="text-align:center;color:#dc2626;padding:24px">Erro ao carregar as fichas. Tente novamente.</div>';
    }
}

function fichaGeralTodasFotos() {
    const jobs = fichaGeralCache.map(x => getFoto(x.c.cpf).catch(() => null));
    return Promise.all(jobs);
}

async function fichaGeralAbrirJanela(imprimirAgora) {
    if (!fichaGeralCache.length) {
        alert('Selecione o projeto e a turma para gerar as fichas antes de imprimir.');
        return;
    }
    const fotos = await fichaGeralTodasFotos();
    let body = '';
...
function fichaGeralImprimirTodas() {
    fichaGeralAbrirJanela(true);
}

function fichaGeralDownloadTodas() {
    fichaGeralAbrirJanela(false);
}

async function fichaGeralImprimirUma(idx) {
    const x = fichaGeralCache[idx];
    if (!x) return;
    let foto = null;
